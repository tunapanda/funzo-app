...
export default Ember.Component.extend({
  ...
  url: 'http://104.131.84.213/funzo-books/FOO.zip',
  ...
  actions: {
    submit() {
      ...
      let bookID = Math.random().toString(36).substring(7);
      let destDir = window.cordova.file.externalDataDirectory || window.cordova.file.dataDirectory;
      let zipDest = `${destDir}tmp/${bookID}.zip`;
      let booksDir = `${destDir}books/`;

      this.set('status', 'downloading');
      Ember.RSVP.all([
        this.createDirectory(destDir, 'tmp'),
        this.createDirectory(destDir, 'books')
      ])
      .then(() => this.fileTransfer(this.get('url'), `${destDir}tmp`, `${bookID}.zip`))
      // .then(() => this.createDirectory(booksDir, bookID))
      .then(() => {
        this.set('status', 'unzipping');

        return this.unzip(zipDest, `${booksDir}`);
      })
      .then(() => {
